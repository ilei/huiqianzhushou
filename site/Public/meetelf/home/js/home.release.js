...
//处理重置按钮click
    ...
    //审核中
    $('#act-verifyed-t').click(function () {
        //$(this).removeClass('disabled').addClass('disabled');
        var keys = $('#input-keyword-search').val().trim();
        //$('#pager-pre-next').removeClass('show').addClass('hidden');
        //$('#act-next').removeClass('disabled').addClass('');
        getActList(4, $('#act-verifyed'), 1, keys, $(this));
        //$('#act-page').val("1");
        $('#act-tab-status').val("4");
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 'ok') {
                ...
                $('#act-verifyed-t').text('审核中(' + data.countverify + ')');
                console.log(data.countverify);

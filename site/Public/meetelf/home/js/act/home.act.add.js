define(['jquery', 'messageControl', 'MTFCropper', 'validate', 'cookie', 'pin', 'datepicker', 'combobox', 'ueditorall', 'ueditorconfig', 'homeCommon', 'lang/l.home.act.add'], function ($, messager) {
    ...
    $("#container").blur(function () {
        console.log('haha');
        //var content_val = $("#container").val();
        var content_val = $($('#container').find('#edui1').find('#ueditor_0').prop('contentWindow').document).find('body').find('p').html();
        $(this).text(content_val);
        console.log($(this).html());
        if (isNaN(content_val)) {
            $("ueditor-error").text("内容不能为空");
        } else {
            $("ueditor-error").text("");
        }
    });
    ...
    $("#act_form").delegate("select[name='area_2']", 'change', function () {
        $.cookie('area_2', $("select[name='area_2']").val());
    });
    ...
    function linkage(s, d) {
        ...
        $.ajax({
            ...
            if (res.status) {
                ...
                if ($.cookie('area_2')) {
                    $("select[name='area_2']").find("option[value='" + $.cookie('area_2') + "']").attr("selected", true);
                }
            } else {
    ...
    $(".mtelf-table-border").delegate("input[name*='name']", 'keyup', function () {
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            if (!$(element).val()) {
                $(element).focus();
                $(".act-save, .act-pub").attr('disabled', true);
    ...
    $(".act-save .act-pub").on("click", function () {
        ...
        //禁用按钮
        $(".act-save").button('loading');
        $(".act-pub").button('loading');
        $(".act-save").attr('disabled', 'disabled');
        $(".act-pub").attr('disabled', 'disabled');
        $("#act_form").submit();
    });
    ...
    $("#act_form").validate({
        ...
        submitHandler: function (form) {
            ...
            $(".act-save").prop('disabled', true);
            $(".act-pub").prop('disabled', true);
            // var content_val = $("#container").val();
            ...
            if ($("#status_post").val() == 1) {
                ...
                messager.show({
                    ...
                    if (type === "submit") {
                        ...
                        $(".act-save").prop('disabled', false);
                        $(".act-pub").prop('disabled', false);
                    }
                });
            ...
            function realSubmit() {
                ...
                $("textarea[name='content']").text($($('#container').find('#edui1').find('#ueditor_0').prop('contentWindow').document).find('body').find('p').html());
                ;
                $.ajax({
                    type: 'POST',

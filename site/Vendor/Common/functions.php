...
function load_config($file,$parse=CONF_PARSE){
    if(APP_DEBUG){
        if(preg_match('/Debug\/[a-zA-Z0-9-_.]+php$/', $file) !== false){
            $pos  = strrpos($file, '/');
            $file = substr($file, 0, $pos) . '/Debug'. substr($file, $pos); 
        }
        if(!file_exists($file)){
            $pos  = strrpos($file, '/');
            $file = substr($file, 0, $pos-6) . substr($file, $pos); 
        }
    }
 ...
 * @param boolean $is_token 是否生成token，只在home模块下生效
 * @return string
 */
function U($url = '', $vars = '', $suffix = true, $domain = false, $is_token = true)
{
    // 解析URL
    $info   =  parse_url($url);
    ...
    if(isset($host)) {
        ...
        $domain = isset($info['host']) && $info['host'] ? $info['host'] : $_SERVER['HTTP_HOST'];
        if(C('APP_SUB_DOMAIN_DEPLOY')) { // 开启子域名部署
            // $domain = $domain=='localhost'?'localhost':'www'.strstr($domain,'.');
            // '子域名'=>array('模块[/控制器]');
            $rules = (APP_DEBUG==true) ? C('APP_SUB_DOMAIN_RULES_DEBUG') : C('APP_SUB_DOMAIN_RULES');
...
//                    $domain = $key.strstr($domain,'.'); // 生成对应子域名
    ...
    // TODO: token放URL里了, 求更NB的方法
    if(MODULE_NAME == 'Home' && $is_token == true) {
        $vars['token'] = session('auth')['guid'];
    }
    if(MODULE_NAME == 'Signin' && $is_token == true) {
        $vars['token'] = session(C('auth_session_name'))['token'];
    }

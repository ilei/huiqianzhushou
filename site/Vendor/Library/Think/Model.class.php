...
class Model {
    ...
    public function find($options=array()) {
        ...
        //活动状态修改 只影响活动其余均不影响，请大家放心使用
        $data   = $this->_change_activity_status($data);
    ...
    protected function _change_activity_status($res){
       if($this->trueTableName == 'ym_activity' && $res && $res['published_at']){
            $time = time();
            if($res['start_time'] <= $time && $res['end_time'] > $time && !in_array($res['status'], array(1,3)) && !in_array($res['is_verify'], array(0,3))){
                $res['status'] = $update['status'] = 1; 
            }elseif($res['end_time'] < $time && $res['status'] != 3){
                $res['status'] = $update['status'] = 2; 
            }
            if($update){
                M('Activity')->where(array('guid' => $res['guid']))->save($update);
            }
       }
       return $res;
    }
